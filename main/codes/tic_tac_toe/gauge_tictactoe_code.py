...
from main.codes.tic_tac_toe.detectors.TicTacToeDrumBlueprint import (
    TicTacToeDrumBlueprint,
)
...
class GaugeTicTacToeCode(ToricHexagonalCode):
    def __init__(
        self, distance: int, bare_tic_tac_toe_route: TicTacToeRoute, gauge_factor
    ):
        ...
        super().__init__(rows=rows, columns=columns, distance=distance)

        self.letters = [PauliLetter("X"), PauliLetter("Y"), PauliLetter("Z")]

        self.gauge_factor = gauge_factor
        self.bare_tic_tac_toe_route = bare_tic_tac_toe_route

        self.tic_tac_toe_route = []
        for _ in range(2):
            for edge in bare_tic_tac_toe_route:
                self.tic_tac_toe_route.extend([edge] * gauge_factor)
        ...
        # need to double check schedule
        # check_schedule = [check for check in check_schedule for _ in range(2)]
        stabilizers, relearned = self.find_stabilized_plaquettes()
        detector_blueprints = self.plan_detectors(stabilizers, relearned)
        if self.gauge_factor == 2:
            detector_schedule = self.create_detectors_g2(detector_blueprints, borders)
        elif self.gauge_factor == 3:
            detector_schedule = self.create_detectors_g3(detector_blueprints, borders)
        elif self.gauge_factor == 4:
            detector_schedule = self.create_detectors_g4(detector_blueprints, borders)

        detector_schedule = self.create_gauge_detectors(
            detector_schedule, check_schedule
        )
    ...
    def find_stabilized_plaquettes(self):
        ...
        length = len(self.bare_tic_tac_toe_route)
        ...
        for t in range(repeats_after):
            edge_colour, edge_letter = self.bare_tic_tac_toe_route[t % length]
            ...
            if t > 0:
                ...
                self.carry_over_plaquettes(t, stabilized, edge_colour, edge_letter)
    ...
    def reinfer_plaquettes(
        self, t, stabilized, relearned, relearned_plaquettes, anti_commuting_plaquettes
    ):
    ...
    def carry_over_plaquettes(self, t, stabilized, edge_colour, edge_letter):
        ...
        for colour, letter in unaffected_plaquettes:
            stabilized[t][(colour, letter)] = stabilized[t - 1][(colour, letter)]

    def remove_plaquettes(self, t, stabilized, edge_colour, edge_letter):
        anti_commuting_plaquettes = rest_of_column(edge_colour, edge_letter)
    ...
    def plan_detectors_of_type(self, colour, letter, stabilizers, relearned):
        ...
        while t < stop:
            ...
            if len(stabilizing_edges) == 0:
                ...
                detector_face = [((t + v - u), c, l) for v, c, l in stabilizing_edges]
                if floor is None:
    ...
    def create_detectors_g3(self, detector_blueprints, borders):
        detectors: List[List[Drum]] = [[] for _ in range(len(self.tic_tac_toe_route))]
            ...
            for anchor in anchors:
                for blueprint in detector_blueprints[colour]:
                    ...
                    for (t, edge_colour, edge_letter) in blueprint.lid:
                        ...
                        lid.extend((t * 2, check) for check in checks)

                    for index, (t, edge_colour, edge_letter) in enumerate(
                        blueprint.floor
                    ):
                        checks = borders[anchor][(edge_colour, edge_letter)]
                        if index == 2:
                    ...
                    detector = Drum(
                        floor,
                        lid,
                        blueprint.learned * self.gauge_factor,
                        drum_anchor,
                    )
                    detectors[blueprint.learned * self.gauge_factor].append(detector)

                    detector = Drum(
                        floor,
                        lid,
                        blueprint.learned * self.gauge_factor
                        + len(self.tic_tac_toe_route) // 2,
                        drum_anchor,
                    )
    ...
    def create_detectors_g4(self, detector_blueprints, borders):
        detectors: List[List[Drum]] = [[] for _ in range(len(self.tic_tac_toe_route))]
        ...
        for colour in self.colours:
            ...
            for anchor in anchors:
                for blueprint in detector_blueprints[colour]:
                    ...
                    for (t, edge_colour, edge_letter) in blueprint.lid:
                        checks = borders[anchor][(edge_colour, edge_letter)]
                        lid.extend((t * self.gauge_factor, check) for check in checks)
                    ...
                    detector = Drum(
                        floor,
                        lid,
                        blueprint.learned * self.gauge_factor + 1,
                        drum_anchor,
                    )
                    detectors[blueprint.learned * self.gauge_factor + 1].append(
                        detector
                    )
    ...
    def create_detectors_g2(self, detector_blueprints, borders):
        detectors: List[List[Drum]] = [[] for _ in range(len(self.tic_tac_toe_route))]
                ...
                for blueprint in detector_blueprints[colour]:
                    ...
                    for index, (t, edge_colour, edge_letter) in enumerate(
                        ...
                        if index == 2:
                            ...
                            floor.extend(
                                (
                                    t * self.gauge_factor + addition_factor,
                                    check,
                                )
                                for check in checks
                            )
    ...
    def create_gauge_detectors(self, detectors, check_schedule):
        ...
        while t < stop:
            if (
                self.tic_tac_toe_route[t % length_tic_tac_toe_route]
                == self.tic_tac_toe_route[(t - 1) % length_tic_tac_toe_route]
            ):
    ...
    def get_init_logical_qubits(self):
        ...
        logical_0 = TicTacToeLogicalQubit(
            vertical_pauli_letter=PauliLetter("Z"),
            horizontal_pauli_letter=PauliLetter("X"),
            code=self,
        )
        logical_1 = TicTacToeLogicalQubit(
            vertical_pauli_letter=PauliLetter("X"),
            horizontal_pauli_letter=PauliLetter("Z"),
            code=self,
        )
