...
class GaugeFloquetColourCode(GaugeTicTacToeCode):
    def __init__(self, distance: int, gauge_factor: int, boundary_repetitions: str = 'single'):
        """A gauge-fixed Floquet colour code.

        Args:
            distance: The distance of the code.
            gauge_factor: The number of times each check is repeated.
        ...
        if boundary_repetitions == 'double':
    ...
    def set_triple_final_detector_schedule(self):
        ...
        for anchor in anchors:            
            ...
            for (t, edge_colour, edge_letter) in red_x_drum_blueprint.floor:
                checks = self.ungauged_code.borders[anchor][(edge_colour, edge_letter)]
        ...
        self.final_check_schedule = [self.ungauged_code.check_schedule[0], self.ungauged_code.check_schedule[2], self.ungauged_code.check_schedule[4], final_checks]
        ...
        for check in (self.final_check_schedule[-4]):
            ...
            final_detectors.append(Drum([(-3, check)], [(0, check_lid_0), (0, check_lid_1)], 2))
        
        for check in (self.final_check_schedule[-3]):
            qubits = list(check.paulis.values())
            check_lid_0 = qubit_check_dic[qubits[0].qubit]
            check_lid_1 = qubit_check_dic[qubits[1].qubit]
            final_detectors.append(Drum([(-2, check)], [(0, check_lid_0), (0, check_lid_1)], 2))

        for check in (self.final_check_schedule[-2]):
            qubits = list(check.paulis.values())
            check_lid_0 = qubit_check_dic[qubits[0].qubit]
            check_lid_1 = qubit_check_dic[qubits[1].qubit]
            final_detectors.append(Drum([(-1, check)], [(0, check_lid_0), (0, check_lid_1)], 2))
    ...
    def set_double_final_detector_schedule(self):
        ...
        for pauli in final_measurements:
            ...
            final_checks.append(check)
            qubit_check_dic[pauli.qubit] = check
        self.final_check_schedule = [self.ungauged_code.check_schedule[0], self.ungauged_code.check_schedule[2],final_checks]
