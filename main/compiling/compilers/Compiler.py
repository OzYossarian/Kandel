...
class Compiler(ABC):
    ...
    def compile_code(
        ...
        self.compile_final_measurements(
            final_measurements, logical_observables, layer, tick, circuit, code
        )

        return circuit.to_stim(self.noise_model)

    def compile_initialisation(self, code: Code, initial_states: Dict[Qubit, State]):
    ...
    def compile_layer(
        ...
        observables: List[LogicalOperator],
        tick: int,
        circuit: Circuit,
        code: Code,
    ):
    ...
    def compile_final_detectors(
        self, final_checks: Dict[Qubit, Check], layer: int, circuit: Circuit, code: Code
    ):
        final_detectors = []
        round = layer * code.schedule_length
        for detector in code.detectors:
            ...
            if open_lid:
                ...
                detector_product = PauliProduct(
                    [pauli for _, check in detector_checks for pauli in check.paulis]
                )
                detector_qubits = {pauli.qubit for pauli in detector_product.paulis}
                measurements = [
                    final_checks[data_qubit].paulis[0] for data_qubit in detector_qubits
                ]
