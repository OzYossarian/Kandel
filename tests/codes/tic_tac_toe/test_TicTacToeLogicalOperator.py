...
def test_update():
    ...
    coords_of_checks = set(
        (pauli.qubit.coords for check in checks_t0 for pauli in check.paulis.values())
    )
    ...
    coords_of_checks = set(
        (pauli.qubit.coords for check in checks_t1 for pauli in check.paulis.values())
    )
    pauli_letter_of_checks = set(
        (pauli.letter for check in checks_t1 for pauli in check.paulis.values())
    ).pop()
    ...
    checks_t2 = logical_op_gauge_hcc_X_hor.update(3)
    coords_of_checks = set(
        (pauli.qubit.coords for check in checks_t2 for pauli in check.paulis.values())
    )
...
def test_at_round():
    ...
    coords_of_logical = set(
        (pauli.qubit.coords for pauli in logical_op_hcc_X_hor.at_round(1))
    )
    ...
    for i in range(3):
        coords_of_logical = set(
            (pauli.qubit.coords for pauli in logical_op_gauge_hcc_X_hor.at_round(i))
        )
        x_log_op_pauli_letter = set(
            pauli.letter.letter for pauli in logical_op_gauge_hcc_X_hor.at_round(i)
        ).pop()
    ...
    logical_qubit_gauge_hcc_X_vert = TicTacToeLogicalQubit(
    ...
    for i in range(10):
        coords_of_logical = set(
            (pauli.qubit.coords for pauli in logical_op_gauge_hcc_Z_hor.at_round(i))
        )
