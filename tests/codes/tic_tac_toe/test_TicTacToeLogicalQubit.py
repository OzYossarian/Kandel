...
logical_qubit_hcc_X_vert = TicTacToeLogicalQubit(
    vertical_pauli_letter=PauliLetter("X"),
    horizontal_pauli_letter=PauliLetter("Z"),
    code=hcc_code,
)
logical_qubit_hcc_Z_vert = TicTacToeLogicalQubit(
...
logical_qubit_gauge_hcc_Z_vert = TicTacToeLogicalQubit(
    vertical_pauli_letter=PauliLetter("Z"),
    horizontal_pauli_letter=PauliLetter("X"),
    code=gauge_hcc_code,
)
...
logical_qubit_hcc_p_X_vert = TicTacToeLogicalQubit(
    vertical_pauli_letter=PauliLetter("X"),
    horizontal_pauli_letter=PauliLetter("Z"),
    code=permuted_code,
)
logical_qubit_hcc_p_Z_vert = TicTacToeLogicalQubit(
    vertical_pauli_letter=PauliLetter("Z"),
    horizontal_pauli_letter=PauliLetter("X"),
    code=permuted_code,
)
...
logical_qubit_fcc_X_vert = TicTacToeLogicalQubit(
    vertical_pauli_letter=PauliLetter("X"),
    horizontal_pauli_letter=PauliLetter("Z"),
    code=fcc_code,
)
logical_qubit_fcc_Z_vert = TicTacToeLogicalQubit(
    vertical_pauli_letter=PauliLetter("Z"),
    horizontal_pauli_letter=PauliLetter("X"),
    code=fcc_code,
)
...
def test_get_initial_X_and_Z_boson():
    (
        ...
        x_initial_boson,
        z_initial_boson,
    ) = logical_qubit_hcc_X_vert.get_initial_X_and_Z_boson()
    ...
    (
        ...
        x_initial_boson,
        z_initial_boson,
    ) = logical_qubit_hcc_Z_vert.get_initial_X_and_Z_boson()
    assert X_equivalent_bosons == [(Green, PauliLetter("X")), (Blue, PauliLetter("X"))]
    assert Z_equivalent_bosons == [(Red, PauliLetter("Y")), (Red, PauliLetter("Z"))]
    assert x_initial_boson == (Green, PauliLetter("X"))
    assert z_initial_boson == (Red, PauliLetter("Y"))
    ...
    (
        ...
        x_initial_boson,
        z_initial_boson,
    ) = logical_qubit_hcc_p_X_vert.get_initial_X_and_Z_boson()
...
def test_get_next_bosons():
    ...
    X_pauli_letters_at_rounds = []
    for i in range(0, 11):
        X_pauli_letters_at_rounds.append(
            logical_qubit_gauge_hcc_Z_vert.get_next_bosons(i)[0][1].letter
        )
    assert X_pauli_letters_at_rounds == [
        "X",
        "X",
        "X",
        "Z",
    ...
    for i in range(0, 5):
        pauli_letters_at_rounds.append(
            logical_qubit_hcc_p_X_vert.get_next_bosons(i)[0][1].letter
        )
    ...
    for i in range(0, 5):
        pauli_letters_at_rounds.append(
            logical_qubit_hcc_p_Z_vert.get_next_bosons(i)[0][1].letter
        )
    assert pauli_letters_at_rounds == ["X", "X", "Z", "Z", "Y"]
    ...
    for i in range(-1, 5):
        pauli_letters_at_rounds.append(
            logical_qubit_fcc_X_vert.get_next_bosons(i)[0][1].letter
        )
    ...
    for i in range(-1, 5):
        pauli_letters_at_rounds.append(
            logical_qubit_fcc_Z_vert.get_next_bosons(i)[0][1].letter
        )
    assert pauli_letters_at_rounds == ["X", "X", "X", "X", "X", "X"]
    ...
    for i in range(-1, 5):
        pauli_letters_at_rounds.append(
            logical_qubit_hcc_X_vert.get_next_bosons(i)[1][1].letter
        )
...
def test_get_initial_horizontal_operator():
    x_log_op = logical_qubit_hcc_Z_vert.get_initial_horizontal_operator(
        PauliLetter("X"), (Blue, PauliLetter("X"))
    )
    ...
    z_log_op = logical_qubit_hcc_X_vert.get_initial_horizontal_operator(
    ...
    x_log_op = logical_qubit_hcc_p_Z_vert.get_initial_horizontal_operator(
        PauliLetter("X"), (Blue, PauliLetter("X"))
    )
    x_log_op_coords = set(pauli.qubit.coords for pauli in x_log_op.at_round(-1))
    x_log_op_pauli_letter = set(pauli.letter.letter for pauli in x_log_op.at_round(-1))
    ...
    x_log_op = logical_qubit_hcc_p_X_vert.get_initial_horizontal_operator(
        PauliLetter("Z"), (Green, PauliLetter("Z"))
    )
    x_log_op_coords = set(pauli.qubit.coords for pauli in x_log_op.at_round(-1))
    x_log_op_pauli_letter = set(pauli.letter.letter for pauli in x_log_op.at_round(-1))
    ...
    x_log_op = logical_qubit_gauge_hcc_Z_vert.get_initial_horizontal_operator(
        PauliLetter("X"), (Green, PauliLetter("X"))
    )
