...
def test_init_circuit_level_noise():
    ...
    assert math.isclose(noise_model.initialisation.px,
                        1 / 3 * 0.17, rel_tol=1e-5)
    assert math.isclose(noise_model.initialisation.py,
                        1 / 3 * 0.17, rel_tol=1e-5)
    assert math.isclose(noise_model.initialisation.pz,
                        1 / 3 * 0.17, rel_tol=1e-5)
